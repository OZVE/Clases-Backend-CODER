...
router.get("/", (req, res) => {
    ...
    knex("carrito").then(carrito => {
        knex('relprod').join("lista", "relprod.idproducto", "=", "lista.id")
        .select("lista.id", "relprod.date", "relprod.qty", "lista.name", "lista.description", "lista.code", "lista.picture", "lista.price" )
        .then(data => {
            let elemCarritos = []
                        ...
                        knex("relprod").select("idproducto").where("idproducto", "=", id).then(data => {
                            if (data.length > 0) {
                                ...
                                knex('relprod').select('*').where("idproducto", "=", id).then(_ => {
                                    knex('relprod').join("lista", "relprod.idproducto", "=", "lista.id").select("lista.id", "relprod.date", "relprod.qty", "lista.name", "lista.description", "lista.code", "lista.picture", "lista.price" )
                                    .where({"lista.id": id}).then(data => {
                                        let elemCarritos = []
                                            data.forEach(element => {
                                                let item = {
                                                    id_carrito: carrito[0].id,
...
router.delete("/:id_producto", (req, res) => {
    ...
    try{
        let id = parseInt(req.params.id_producto)
           ...
           knex("relprod").where("idproducto", "=", id).then(data => {
               if(data.length > 0){
                   knex('relprod')
                   .where("idproducto", "=", id)
                   .del().then(() =>  res.json("Producto eliminado del carrito con éxito"))
               } else {
                   res.json({msg: "El producto con el id " + id + " no existe en el carrito"})
               }
           })
