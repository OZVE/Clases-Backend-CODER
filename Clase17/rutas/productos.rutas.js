...
router.get("/:id", (req, res) => { 
   knex('lista').where({'id': req.params.id}).then((data) =>  res.json(data[0]))
})
...
router.post("/", (req, res) => {
    ...
    try{
            knex('lista').insert({
              ...
              }).then( id => knex('lista').where({'id': id[0]}).then((data) =>  res.json(data[0])))
...
router.delete("/:id", (req, res) => {
    if(admin){
        try{
            let id = parseInt(req.params.id) 
               knex("lista").where({"id":id}).then(data => {
                   if(data.length > 0){
                       knex('lista').where({ id: id})
